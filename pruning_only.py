...
def main(config):
    ...
    with tqdm(range(0,1000,1)) as pbar:
        ...
        for prune_rate in pbar:
            ...
            prune_loop(model, criterion, pruner, train_dataloader, 'cuda', sparsity=prune_rate, schedule=config.schedule, scope=config.scope,\
            ...
            torch.save(save_dict, config.root/run_name/f"{prune_rate}.pth")
